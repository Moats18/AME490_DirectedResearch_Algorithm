function x_tessellated = tessellate2D(x, l1, l2, n)

% Function to tessellate a set of vertices in R2 by Bravais lattice vectors
%
% By: Andrew Song
...
%
% Inputs:
% x: coordinates of vertices, 2N by 1 vector where N is # of vertices
% l1: horizontal Bravais lattice vector [l1_x l1_y]
% l2: vertical Bravais lattice vector [l2_x l2_y]
% n: # of tessellations (if n=0 -> tessellate2D is identity)
...
N = length(x)/2; % number of vertices
x_new = zeros(N, 2);
for i = 1:N
    x_new(i,1) = x(2*i-1, 1);
    x_new(i,2) = x(2*i, 1);

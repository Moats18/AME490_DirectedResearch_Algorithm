function Ropt = rotationMin_2D(x, y, Fj, Tj, J)
% 
% uses quaternions to convert the minimization into an eigenvalue probelm, 
...
for j = 1:length(J)
    ...
    for i = 1:length(Fj(:, :, j))
       ...
       ckl_temp = y(2*k-1:2*k, 1) - cj{j};
       ckl = [ckl_temp(1); ckl_temp(2); 0]; % convert to R3 for SO3 rotation compatibility
       ckl_cross = [0         0         ckl(2);
                    0         0         -ckl(1);
                    -ckl(2)   ckl(1)    0];
...
for j = 1:length(J)
    ...
    v_cross = [0       -v(3) v(2);
               v(3)    0     -v(1);
               -v(2)   v(1)  0];
    R3{j} = v*v.' + qr^2*eye(3) + 2*qr*v_cross + v_cross*v_cross;
    R{j} = [R3{j}(1,1)   R3{j}(1,2);
            R3{j}(2,1)   R3{j}(2,2)];
end
